private ["_buildings","_building","_gotbuilding","_price","_lease","_sell","_totaloccupants","_size"];
...
if(_gotbuilding) then {
	...
	_ret = [_building,_ret select 0,_ret select 1,_ret select 2,_ret select 3,_ret select 4];
