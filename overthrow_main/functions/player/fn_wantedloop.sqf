...
if !(captive _unit) then {
	...
	if(_unit call OT_fnc_unitSeenCRIM && !_gottem) then {
		...
		if !(_i isEqualTo -1) then {
			...
			if(_gangid > -1) then {
				...
				if(_rep < -9) then {
					...
					if(isPlayer _unit) then {
						private _lasttime = _unit getvariable ["lastreco",0];
						private _time = time;
						if(_time - _lasttime >= 300) exitwith {//Incase the players spawn is immeditly visible by the gang, give them a chance to escape
							format["%1 have recognized you",_name] call OT_fnc_notifyMinor;
							_unit setCaptive false;
							[_unit] call OT_fnc_revealToCRIM;
							_unit setVariable ["lastreco",_time];
						};
					}else{
						_unit setCaptive false;
						[_unit] call OT_fnc_revealToCRIM;
				...
				if(_rep < 10) then {
					...
					if(!(_vehicle isEqualTo _unit) && { _unit call OT_fnc_illegalInCar } && _rep < 30) exitWith {
					...
					if (_unit call OT_fnc_hasWeaponEquipped && _rep < 30) exitWith {
						if(isPlayer _unit) then {
		...
		if(_unit call OT_fnc_unitSeenNATO && !_gottem) then {
			...
			if (!(hmd _unit isEqualTo "") && { !(hmd _unit isEqualTo "ACE_NVG_Gen1") }) exitWith {
				if(isPlayer _unit) then {
			...
			if(isPlayer _unit) then { //Who dosn't like a random search
				[] spawn OT_fnc_randomsearch;
			};
			private _unitpos = getPosATL _unit;
			private _base = _unitpos call OT_fnc_nearestObjAndFob;
