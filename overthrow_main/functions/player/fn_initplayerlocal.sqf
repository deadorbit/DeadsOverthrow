...
private _maxTime = diag_tickTime + 60;
...
if(!isserver) then {
	...
	//null = execVM "\overthrow_main\fuuctions\aidrivingcontrol\init.sqf";
...
if (_newplayer) then {
    _clothes = (OT_clothes_locals call BIS_fnc_selectRandom);
	...
	private _bag = selectRandom OT_starterBackpacks;
...
//Create ACE class interactions

//Ammobox interactions

//Parent interaction
	_condition = {
		([_target, _player, []] call ace_common_fnc_canInteractWith)
	};
	_statement = {};
	_action = ["OT Cargo","OT Cargo","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_Storage), 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;
...
//Dump stuff
	...
	_statement = {
		[_player, _target] remoteexec ["OT_fnc_dumpStuff",_player,false];
	};
	_action = ["Dump Everything","Dump Everything","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_Storage), 0, ["ACE_MainActions","OT Cargo"], _action] call ace_interact_menu_fnc_addActionToClass;

//Dump stuff warehouse
	_condition = {([_target, _player, []] call ace_common_fnc_canInteractWith && call OT_fnc_playerIsAtWarehouse)};
	_statement = {[_player] remoteexec ["OT_fnc_dumpIntoWarehouse",_player,false];};
	_action = ["Dump Everything (Warehouse)","Dump Everything (Warehouse)","",_statement,_condition] call ace_interact_menu_fnc_createAction;
	
[(OT_item_Storage), 0, ["ACE_MainActions","OT Cargo"], _action] call ace_interact_menu_fnc_addActionToClass;

//lock

	_condition = {([_target, _player, []] call ace_common_fnc_canInteractWith && !(_target getVariable ['OT_locked',false]))};
	_statement = {
		_target setVariable ["OT_locked",true,true];
		"Ammobox Locked!" remoteexec ["OT_fnc_notifyMinor",_player,false];
	};
	_action = ["Lock Ammobox","Lock Ammobox","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_Storage), 0, ["ACE_MainActions","OT Cargo"], _action] call ace_interact_menu_fnc_addActionToClass;

//unlock

	_condition = {([_target, _player, []] call ace_common_fnc_canInteractWith && _target call OT_fnc_playerIsOwner && _target getVariable ['OT_locked',false])};
	_statement = {
		_target setVariable ["OT_locked",false,true];
		"Ammobox Unlocked!" remoteexec ["OT_fnc_notifyMinor",_player,false];
	};
	_action = ["Unlock Ammobox","Unlock Ammobox","",_statement,_condition] call ace_interact_menu_fnc_createAction;
...
//Take from warehouse
	...
	_action = ["Take From warehouse","Take From warehouse","",_statement,_condition] call ace_interact_menu_fnc_createAction;
...
//Safe parent interaction menu
	_condition = {
		([_target, _player, []] call ace_common_fnc_canInteractWith)
	};
	_statement = {};
	_action = ["OT Money","OT Money","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_safe), 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;

//Store money in safe interaction
	_condition = {
		([_target, _player, []] call ace_common_fnc_canInteractWith)
	};
	_statement = {
		[_target] remoteexec ["OT_fnc_safePutMoney",_player,false];
	};
	_action = ["Store Money","Store Money","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_safe), 0, ["ACE_MainActions","OT Money"], _action] call ace_interact_menu_fnc_addActionToClass;

//Take money from safe
	_condition = {
		([_target, _player, []] call ace_common_fnc_canInteractWith)
	};
	_statement = {
		[_target] remoteexec ["OT_fnc_safeTakeMoney",_player,false];
	};
	_action = ["Take Money","Take Money","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_safe), 0, ["ACE_MainActions","OT Money"], _action] call ace_interact_menu_fnc_addActionToClass;

//Put password on safe
	_condition = {
		([_target, _player, []] call ace_common_fnc_canInteractWith)
	};
	_statement = {
		[_target] remoteexec ["OT_fnc_safeSetPassword",_player,false];
	};
	_action = ["Set Password","Set Password","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[(OT_item_safe), 0, ["ACE_MainActions","OT Money"], _action] call ace_interact_menu_fnc_addActionToClass;
...
//container interactions
	...
	_statement = {
		[_player,["gear",_target]] remoteexec ["action",_player,false];
	};
	_action = ["Accese Inventory","Accese Inventory","",_statement,_condition] call ace_interact_menu_fnc_createAction;
[("OT_Land_Cargo10_grey_F"), 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToClass;
...
//_index = player createDiarySubject ["page1","Manuel"];

//cfgmusic config name
