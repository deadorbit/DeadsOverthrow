...
if ((date select 3) != _lasthr) then {
	...
	private _wages = 0;
	{
		if(_x != "Factory") then {
			...
			{
				if ((_x select 0) == _Business) then {_target = (_x select 1)};
			} forEach OT_BusinessLevel;
			
			
			
			if(_enum > 20 * _target) then {
				_enum = 20 * _target;
			};
			...
			if(_funds >= _towage) then {
				...
				if(_num > 0) then {
					...
					if(count _data isEqualTo 4) then { //Turns one thing into another
						_input = _data select 2;
					...
					if(count _data isEqualTo 5) then { //Turns two things into one other
						_input = _data select 2;
						_inputa = _data select 3;
						_output = _data select 4;
						_container = _pos nearestObject OT_item_CargoContainer;
						if(_container isEqualTo objNull) then {
							_p = _pos findEmptyPosition [5,100,OT_item_CargoContainer];
							_container = OT_item_CargoContainer createVehicle _p;
						...
						if(_input != "") then {
							...
							_inputanum = 0;
							{
								...
								{
									_x params ["_cls","_amt"];
									if(_cls isEqualTo _input) then {
										if(_amt >= _innum) then { // If their are more items then amount of workers
											_inputnum = _inputnum + _innum; //Amount equals 0 + number of workers
										}else{ //More workers than items
											//_innum = _innum - _amt; //Number to make equals demand minus amount of item?
											_inputnum = _inputnum + _amt;
										};
									};
									if(_cls isEqualTo _inputa) then {
										if(_amt >= _innum) then {
											_inputanum = _inputanum + _innum;
										}else{
											//_innuma = _innuma - _amt;
											_inputanum = _inputanum + _amt;
										};
									};
								}foreach(_c call OT_fnc_unitStock);
							}foreach(_pos nearObjects [OT_item_CargoContainer, 50]);
							if (_inputnum > _inputanum) then {
								_inputnum = _inputanum;
							};
							_outnum = round (_outnum * (_inputnum / _intotal));
							[_container, _input, _inputnum] call CBA_fnc_removeItemCargo;
							[_container, _inputa, _inputnum] call CBA_fnc_removeItemCargo;
							_container addItemCargoGlobal [_output,_outnum];
						};
					};
				};
...
if ((date select 4) != _lastmin) then {
	...
	_revealed = server getVariable ["revealedFOBs",[]];
	{
		...
		if (!(_id in _revealed) && (_support > (random [500,2000,5000]))) then {
	...
	if("Factory" in (server getVariable ["GEURowned",[]])) then {
		...
		if(_currentCls != "") then {
			...
			if(count _cost > 0) then {
				...
				if(_steel > 0 && (getnumber(configFile >> "CfgVehicles" >> _currentCls >> "side") == 1)) then {
					_natoparts = ceil(_steel/4);
				};
				if(_steel > 0 && _currentCls iskindof "Air" && !(_currentCls iskindof "UAV_06_base_F" || _currentCls iskindof "UAV_01_base_F" || _currentCls iskindof "UAV_02_Base_lxWS")) then {
				...
				if(_timespent isEqualTo 0) then {
					...
					private _doAGM = ["OT_AGM",_agm,OT_factoryPos] call OT_fnc_hasFromCargoContainers;
					private _donatoparts = ["OT_natoparts",_natoparts,OT_factoryPos] call OT_fnc_hasFromCargoContainers;
					private _doaircraftparts = ["OT_aircraftpart",_aircraftparts,OT_factoryPos] call OT_fnc_hasFromCargoContainers;
					private _domoney = ([] call OT_fnc_resistanceFunds >= _costtoproduce);
					if(_dowood && _dosteel && _doplastic && _donatoparts && _doaircraftparts && _domoney) then {
						...
						["OT_AGM",_agm,OT_factoryPos] call OT_fnc_takeFromCargoContainers;
						["OT_natoparts",_natoparts,OT_factoryPos] call OT_fnc_takeFromCargoContainers;
						["OT_aircraftpart",_aircraftparts,OT_factoryPos] call OT_fnc_takeFromCargoContainers;
						[-_costtoproduce] call OT_fnc_resistanceFunds;
						_timespent = _timespent + 1;
					}else{
                        			_need = "";
			                        if !(_dowood) then {_need = _need + format["%1x wood ",_wood]};
			                        if !(_dosteel) then {_need = _need + format["%1x steel ",_steel]};
			                        if !(_doplastic) then {_need = _need + format["%1x plastic ",_plastic]};
									if !(_donatoparts) then {_need = _need + format["$%1x Nato Vehicle Parts",_natoparts]};
									if !(_doaircraftparts) then {_need = _need + format["$%1x Aircraft Parts",_aircraftparts]};
									if !(_doAGM) then {_need = _need + format["$%1x Artillary Guidance Modules",_agm]};
