...
if ([_topos,_fromregion] call OT_fnc_regionIsConnected) then {
    ...
    };
    {
		...
		if(_foreachindex == 2) then {
			_veh additemcargoglobal ["OT_AGM",1];
			_veh additemcargoglobal ["OT_natoparts",round(random[25,50,100])];
			_veh additemcargoglobal ["OT_aircraftpart",round(random[10,25,50])];
		};
    }foreach(_vehtypes);
