        ...
        if(count _gangs > 0) then {
            ...
            if(count _gang == 9) then { //filter out old gangs
                ...
                if(_stability < 50) then {
                    _add = _add + ((50*_level) - _stability);
                };
				...
				if(_level == 1) then {
					if(_resources > 100 && _numingang < 15) then {
						...
						if(_resources > 1000) then {// Gang levels up if its full and has the resources
							_resources = _resources - 1000;
							[_gangid] call OT_fnc_gangLevelUp;
						};
					};
				}else{
					if(_resources > 200 && _numingang < 24) then {
						if((random 50) > 50) then {
							_resources = _resources - 200;
							[_gangid,2] call OT_fnc_addToGang;
						}else{
							_resources = _resources - 200;
							[_gangid,1,true,"launcher"] call OT_fnc_addToGang;
						};
					};
				};
