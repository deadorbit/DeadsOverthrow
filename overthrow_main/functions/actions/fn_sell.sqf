...
if(isNil "_cls" || {_cls isEqualTo ""} || (_cls isEqualTo "ACE_Foritfy")) exitWith {OT_selling = false};

private _price = [_town,_cls,_standing] call OT_fnc_getSellPrice;

private _incStab = server getvariable [format["%1incStab",_town],14000];

_incStab = _incStab - _price;
if(_incStab <= 0) then {
	server setVariable [format["%1incStab",_town],14000+_incStab];
