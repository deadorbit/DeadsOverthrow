...
private _doSalvage = {
  [_this] spawn {
	...
	private _cls = typeof _wreck;
    private _steel = 1;
    private _plastic = 0;
	private _natoparts = 0;
	private _aircraftparts = 0;
    private _cost = cost getVariable [_cls,[100,0,0,0]];
	_wreck setVariable ["Salvaging", true, true];

	if(_steel > 0 && (getnumber(configFile >> "CfgVehicles" >> _cls >> "side") == 1)) then {
		_natoparts = floor((_cost select 2)/6);
		if(_cls iskindof "UAV_01_base_F") then {
			_steel = 0;
			_plastic = 1;
			_natoparts = 1;
		};
    ...
    closeDialog 0;
	private _toname = _cls call OT_fnc_vehicleGetName;
    ...
    for "_x" from 0 to (_steel - 1) do {
        if(!(_veh canAdd "OT_Steel")) exitWith {
            "Vehicle is full, use a container or ammobox for more storage." call OT_fnc_notifyMinor;
        };
        _done = _done + 1;
        _veh addItemCargoGlobal ["OT_Steel", 1];
    };
    if(_plastic > 0 && (_veh canAdd "OT_Plastic")) then {
	...
	if(_natoparts > 0 && (_veh canAdd "OT_NatoParts")) then {
        _veh addItemCargoGlobal ["OT_NatoParts", _natoparts];
        format["Salvaged: %1 x Nato Vehicle Parts",_natoparts] call OT_fnc_notifyMinor;
	};
    deleteVehicle _wreck;
  };
};

private _objects = player nearEntities [["Car","B_CargoNet_01_ammo_F","Air","Ship","OT_Land_Cargo10_grey_F"],20];
_filtered = [];
{
