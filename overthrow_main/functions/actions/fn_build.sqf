...
if ((!_isbase) && !(_closest in (server getVariable ["NATOabandoned",[]]))) exitWith {
	if(_isobj) then {
		...
		format ["NATO forbids construction in %1",_closest] call OT_fnc_notifyMinor;
	};
};
	...
	if(_btn isEqualTo 0 && _sx > (safezoneX + (0.1 * safezoneW)) && _sx < (safezoneX + (0.9 * safezoneW))) then {
		...
		if(!isNull modeTarget && canBuildHere) then {
			...
			if(_money < modePrice) then {
				...
				{ _x hideObjectGlobal true } foreach (nearestTerrainObjects [modeValue,["TREE", "SMALL TREE", "BUSH"],10]);
				...
				if(modeCode != "") then {
					...
					[_created,modeValue,modeCode,false] remoteExec ["OT_fnc_initBuilding",2];
				};
