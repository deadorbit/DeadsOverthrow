...
if(_town in (server getVariable ["NATOabandoned",[]]) || OT_adminMode) then {
	_items = OT_Resources + OT_allItems + OT_allBackpacks + ["V_RebreatherIA"] + ["ACE_NVG_Gen1"] + OT_allWeapons + OT_allMagazines + OT_allAttachments + OT_allStaticBackpacks + OT_allOptics + OT_allVests + OT_allHelmets + OT_allClothing;
}else{
...
_numitems = 0;
{
	...
	if(tolower(_cls) find _SearchTerm > -1) then {
		...
		if !((_cls in _done) || (_cls in OT_allExplosives) || (_cls == "OT_NatoParts") || (_cls == "OT_AGM")) then {
