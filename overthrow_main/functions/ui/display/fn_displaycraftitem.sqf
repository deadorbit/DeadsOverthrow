...
if(count _def > 0) then {
    ...
    private _recipeText = "";
    {
        ...
        _name = _rcls call {
            params ["_rcls"];
            if(_rcls isEqualTo "Uniform_Base") exitWith {"Outfit"};// allows hierarchy w/o display name to have display name in recipe menu
			if(_rcls isEqualTo "CA_LauncherMagazine") exitWith {"Rocket"};
			if(_rcls isEqualTo "HandGrenade") exitWith {"Grenade"};
			if(_rcls isEqualTo "SmokeShell") exitWith {"Chemlight"};
    ...
    if(_cls isKindOf ["Default", configFile >> "CfgMagazines"]) then {
		...
		if (_cls isKindOf "Weapon_Bag_Base") then {
			_desc = _cls call OT_fnc_VehicleGetDescription;
			_itemName = _cls call OT_fnc_VehicleGetName;
