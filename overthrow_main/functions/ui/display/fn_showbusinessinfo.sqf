...
private _innum = _num * 1;
private _outnum = _num * 3;
...
_text = _text + format["<t size='0.65'>Level: %1/5</t><br/>",_target];
...
if(count _data isEqualTo 5) then{
	...
	_text = _text + format["<t size='0.65'>Input: %1 x %2 + %3 /hr</t><br/>",_innum, _input call OT_fnc_weaponGetName, _inputA call OT_fnc_weaponGetName];
	...
	if(count _data > 2) then {
		...
		_text = _text + format["<t size='0.65'>Income: $%1 /hr</t><br/>",round(_num * 600)];
	};
};
