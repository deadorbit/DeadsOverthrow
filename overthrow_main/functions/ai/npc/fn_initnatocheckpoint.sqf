...
while {!(isNil "_group") && count (units _group) > 0} do {\
		...
		if !(_unit in _inrange || _unit in _searching || _unit in _searched) then {
			if(_unit call OT_fnc_unitSeenNATO) then {
				...
				if((isPlayer _unit) && (captive _unit)) then {
					if(_iscar) then {
						[_leader, {_this globalchat "Please approach the checkpoint SLOWLY, DO NOT EXIT your vehicle or attempt to FLEE."}] remoteExec ["call", _unit, false];
						_hdl = _unit addEventHandler ["GetOutMan", {
							params ["_unit", "_role", "_vehicle", "_turret","_hdl"];
								_unit setCaptive false;
								[_unit] call OT_fnc_revealToNATO;
						}];
						_unit setVariable ["CheckpointAutoWanted",_hdl];
	...
	_gone = [];
	{
		...
		private _hdl = _x getVariable ["CheckpointAutoWanted",""];
		if(_x distance _start > _outerRange) then {
			...
			if(isPlayer _x && !(_x in _searched)) then {
				...
				if (!(_hdl isEqualTo "")) then {
					_x setVariable ["CheckpointAutoWanted",""];
					_x removeEventHandler ["GetOutMan", _hdl];
				};
			};
		}else{
			_iscar = false;
			...
			if(_x distance _start < _innerRange) then {
				if !(_x in _searching || _x in _searched) then {
					...
					if(isPlayer _x && !(_x in _searched)) then {
						...
						{
                            _cls = _x select 0;
                            if ((_cls in OT_allWeapons + OT_allMagazines + OT_illegalHeadgear + OT_illegalVests + OT_allStaticBackpacks + OT_allOptics + OT_allExplosives) && {!(_cls in OT_legal)}) then {  //changed to allow legal credit MaxP
                                _foundweapons = true;
                            };
						...
						if(_foundillegal || _foundweapons) then {
							...
							if((vehicle _x) getVariable ["stolen",false]) then {
								_msg = "This vehicle has been reported stolen!";
								{
									//_x setCaptive false;
									[_x] call OT_fnc_revealToNATO;
									[_x, {_this setCaptive false}] remoteExec ["call", _x, false];
								}foreach(units vehicle _unit);
							};
						};
...

	{
		...
		if(_x in _searched) then {
			...
			if(isplayer (driver _x)) then {
				private _leader = driver _x;
				//driver _x setCaptive false;
				[_leader] call OT_fnc_revealToNATO;
				[_leader, {_this globalchat "Hey! Someone stop him!"}] remoteExec ["call", _leader, false];
				[_leader, {_this setCaptive false}] remoteExec ["call", _leader, false];
			};
		};
	}foreach(_gone);
	sleep 2;
