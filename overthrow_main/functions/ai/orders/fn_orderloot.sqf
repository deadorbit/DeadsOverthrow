...
private _target = _sorted select 0;
{
	...
	[_x,_target,OT_Looters] spawn {
        ...
        if (_count isEqualTo 1) then { _unit groupchat format["<%1>: Looting bodies within 100m into the %2", name _unit, (typeof _t) call OT_fnc_vehicleGetName]; };
		...
		if (OT_Looters == 0) then {
			_unit groupchat format ["<%1>: We're done here! Let's go!", name _unit select 0];
		};
	};
}foreach(_myunits);