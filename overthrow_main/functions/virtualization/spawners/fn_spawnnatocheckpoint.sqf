...
_civ = _group createUnit [OT_NATO_ReinforcementsLead, _start, [],0, "NONE"];
...

{
	if(_x isKindOf "StaticWeapon") then {
		...
		((units _x) select 0) setSkill ["aimingAccuracy",0.25]; //pew pew
		((units _x) select 0) setSkill ["general",0.25 + random(0.25)]; //pew pew
		sleep 0.5;
	};
	_groups pushback _x;
}foreach(_vehs);
