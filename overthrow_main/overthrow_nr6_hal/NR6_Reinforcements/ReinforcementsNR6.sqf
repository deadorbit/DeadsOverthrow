//[Side (side), Strength (in number of groups per spawn from 1 to 10), Spawning Positions (as array of positions), Spawning Radius (In meters, recommended 100 to 700 depending on Strength. Minimum 10m.), Available Forces (number of groups), Faction (string of faction name), Threshold (from 0 to 1, will try to keep this ratio of troops present), Array of leaders for that side for HAL control (example : [LeaderHQ,LeaderHQB,LeaderHQC]), Source object (obj name), Rejoining position (pos array), active spawn mode (non reinforcements mode, only to spawn randomized forces at that position, boolean), player range to stop spawning (number)]  spawn NR6_fnc_Reinforcements; 
...
private 
    [
    "_side","_rStrgt","_playerRange","_SpawnPos","_StartForces","_sidetick","_faction","_CurrentForces","_Pool","_Threshold","_SpawnRadius","_Leaders","_SpawnRGroup","_CTR","_ObjSource","_CanSpawn","_RejoinPoint","_SpawnMode","_ThresholdDecay"
    ];

_side = _this select 0;
_rStrgt = _this select 1;
_SpawnPos =  _this select 2;
_SpawnRadius = _this select 3;
_sidetick = _this select 4;
_faction = _this select 5;
_Threshold = _this select 6;
_Leaders = _this select 7;
if ((count _this) >= 10) then {_ObjSource = _this select 9};
if ((count _this) >= 11) then {_RejoinPoint = _this select 10};
_SpawnMode = false;
if ((count _this) >= 12) then {_SpawnMode = _this select 11};
_playerRange = 500;
if not (_SpawnMode) then  {_ThresholdDecay = (1/_sidetick)};
if (_SpawnMode) then {_playerRange = 0};
if ((count _this) >= 13) then {_playerRange = _this select 12};
if ((count _this) >= 14) then {_ThresholdDecay = _this select 13};
if ((count _this) >= 15) then {_ExtraArgs = _this select 14};
if (isNil "_ExtraArgs") then {_ExtraArgs = ""};
...
if (_SpawnMode) exitwith {
    for "_i" from 1 to _rStrgt do
        ...
        [_SpawnPos,_SpawnRadius,_side,_Pool,_Leaders,nil] call SpawnRGroup;
        ...
        for "_i" from 1 to _rStrgt do
            ...
            if (_sidetick > 0) then {
                ...
                if (isNil "_RejoinPoint") then {
                    ...
                    [_SpawnPos,_SpawnRadius,_side,_Pool,_Leaders,_RejoinPoint] call SpawnRGroup;
                };
