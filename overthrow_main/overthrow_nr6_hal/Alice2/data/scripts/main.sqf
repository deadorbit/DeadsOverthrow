...
*/
_temp = [] call compile preprocessFileLineNumbers 'overthrow_nr6_hal\alice2\data\scripts\init-alice2.sqf';
_logic = _this;
_logic setpos [1,1,1];
...
*/
//--- File paths
BIS_Alice2_path = "overthrow_nr6_hal\alice2\data\";
...
//--- Towns faction
_townsFaction = if (isnil {_logic getvariable "townsFaction"}) then {["CIV_F"]} else {_logic getvariable "townsFaction";};
...
for [{_i = 0}, {_i < _totobj}, {_i = _i + 1}] do {
	...
	if (isclass _actual) then {
		...
		if (("Man" in _parents) or ("Car" in _parents)) then {
			...
			//--- Civilians
			if (_class iskindof "civilian" && (_scope == 2) && ("Man" in _parents)) then {
			...
			//--- Civilian Cars
			if (("Car" in _parents) && _side == 3 && _scope == 2) then {
				_faction = gettext (_actual >> "faction");
...
_allActionsx = [];
_allActions = [
	"Alice2\data\actions\safe_stay.fsm",
	"Alice2\data\actions\safe_walk.fsm"
];
