...
if (call OT_fnc_NumVehs > 200) exitWith{};
		...
		if !(_vehtype in OT_vehTypes_civignore) then {
			...
			if (count _roadscon isEqualTo 2) then {
				...
				if(count _posVeh > 0) then {
					...
					private _ran = random 100;
					if(_ran < 5) then {
						_veh addWeaponWithAttachmentsCargo [["rhs_weap_pb_6p9","rhs_acc_6p9_suppressor","","",["rhs_mag_9x18_8_57N181S",8],[],""], 1];
						_veh addMagazineCargo ["rhs_mag_9x18_8_57N181S",2+round(random 3)];
						_veh additemcargo ["OT_Ganja",3+round(random 3)];
					}else{
						if(_ran < 75) then {
							private _itm = ["ItemWatch","ItemCompass","ItemMap","FirstAidKit"] call BIS_fnc_selectRandom;
							_veh additemcargo [_itm,1];
						};
					};
