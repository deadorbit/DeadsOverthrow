...

{
	...
	if(_key == "bases") then {
		{
			...
			_veh setflagtexture OT_flag_IND_txt;
	...
	if(_key == "warehouse") then {//[3,[["negades",[["eo_shirt_checkblk",1]]],["sfaka",[["ACE_WaterBottle",1],["ACE_fieldDressing",2]]]]]
		diag_log _val;
		private _version = _val select 0;
		_val = _val select 1;//[["town",[["item1",1],["item2",2]]],["",[]]]
		if(!(isnil{_val})) then {
			diag_log _val;
			{
				private _town = _x select 0;//["town",[["item1",1],["item2",2]]]
				warehouse setvariable [_town,[],true];
				private _townwarehouse = warehouse getvariable _town;
				private _items = _x select 1;
				{
					private _cls = _x select 0;//[["item1",1],["item2",2]]
					private _amnt = _x select 1;
					private _data = [_cls,_amnt];//["item1",1]
					_townwarehouse pushback _data;
				}foreach(_items);
	...
	if(_key == "vehicles") then {
		...
		_ccc = 0;
		{
			...
			if !(_type isKindOf "Man") then {
				...
				_dir = _x select 2;
				_stock = _x select 3;
				_owner = (_x select 4) select 0;
				_stolen = (_x select 4) select 1;
				_texture = (_x select 4) select 2;
				_name = "";
				if(count _x > 5) then {
					_name = _x select 5;
				};
				...
				*/
				...
				if(_type isKindOf "Building") then {
					{ _x hideObjectGlobal true } foreach (nearestTerrainObjects [(_pos),["TREE", "SMALL TREE", "BUSH"],10]); // Clears any foliage that may be inside the building
				...
				// Otherwise, set the owner (as per usual)
					...
					if(!(isnil{_texture})) then {
						private _idx = 0;
						{
							_veh setobjecttextureglobal [_idx,_x];
							_idx = _idx + 1;
						}foreach(_texture);
					};
				};
				...
				if(count _x > 6) then {
					_code = (_x select 6);
					if(_code != "") then {
						[_veh,getpos _veh,_code,true] call OT_fnc_initBuilding;
					};
	...
	if(_key == "OT_BusinessLevel") then {
		OT_BusinessLevel = _val;
		publicVariable "OT_BusinessLevel";
		_set = false;
	};
	if(_key == "OT_ObjectiveData") then {
		OT_ObjectiveData = _val;
		publicVariable "OT_ObjectiveData";
		_set = false;
	};
	if(_key == "OT_airportData") then {
		OT_airportData = _val;
		publicVariable "OT_airportData";
		_set = false;
	};
	if(_key == "OT_factoryLevel") then {
		OT_factoryLevel = _val;
		publicVariable "OT_factoryLevel";
		_set = false;
	};
	if(_key == "OT_NATOBluprints") then {
		OT_NATOBluprints = _val;
