...
/* 3 */		_s,
/* 4 */		[["", _x call OT_fnc_getOwner] select (_x call OT_fnc_hasOwner),_x getvariable ["stolen",false],getobjecttextures _x],		// Save an empty string if the object doesn't have an owner yet,(Save texture)
...
private _towns = allVariables warehouse;
private _warehousedata = [];
{
	private _append = [_x];
	private _townwarehouse = warehouse getvariable _x;
	_append pushback _townwarehouse;
	_warehousedata pushback _append;
}foreach(_towns);
_warehouse pushback _warehousedata;
...
_data pushBack ["OT_BusinessLevel",OT_BusinessLevel];
_data pushBack ["OT_ObjectiveData",OT_ObjectiveData];
_data pushBack ["OT_airportData",OT_airportData];
_data pushBack ["OT_factoryLevel",OT_factoryLevel];
_data pushBack ["OT_NATOBluprints",OT_NATOBluprints];
