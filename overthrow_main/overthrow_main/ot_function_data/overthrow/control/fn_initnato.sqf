...
//Pregenerate NATO loadouts
{
	if (isNil "_x") then {continue};
	private _unit = _x;
	private _unitLoadout = [];
	private _loadouts = [];
	private _numLoadouts = round(random (OT_maxRandomLoadouts - OT_minRandomLoadouts)) + OT_minRandomLoadouts;
	
	for "_i" from 1 to _numLoadouts do {
		_loadouts pushback ([_unit, _unitLoadout, [], true] call OT_fnc_getRandomLoadout);
	};
	
	spawner setVariable [format["loadouts_%1", _unit], _loadouts, false];
}foreach(OT_NATO_AllUnits);
