...
private _nextturn = 300; //wait 5 minutes from game start until spending resources
private _count = 0;
...
[{
	...
	if(_numplayers > 0) then {
		...
		if(count _schedule > 0) then {
			...
			if(_idx > -1) then {
				...
				if(_mission isEqualTo "CONVOY") then {
					...
					_count = 0;
					_vehtypes = OT_NATO_Vehicles_Convoy;
		...
		//Fix to stop NATO from acting before game has even begun
		if (!(_countered) && !(isnil "OT_economyLoadDone")) then {
			_sorted = [OT_allTowns,[],{server getvariable format["population%1",_x]},"DESCEND"] call BIS_fnc_SortBy;
			{
				...
				if(_pos call OT_fnc_inSpawnDistance) then {
					...
					if(_population >= 100 && {_stability isEqualTo 0} && {!(_town in _abandoned)}) then {
						...
						if(_resources > 250 && _stability < 50 && {!(_town in _abandoned)} && !(server getVariable [format["NATOpatrolsent%1",_town],false])) then {
		...
		
		{
			...
			if(!_done) then {
				...
				if(_ty isEqualTo "P" || _ty isEqualTo "H") then {
					if(_resources > 250 && ((random 100) > _chance)) then {
				...
				if(_ty isEqualTo "V" && _threat > 100) then {
					if(_resources > 200 && ((random 100) > _chance)) then {
		...
		if(time >= OT_nextNATOTurn && {!_countered}) then {
				...
				private _done = false;
				{
					...
					if(_townPos call OT_fnc_inSpawnDistance) then {
						...
						if !(_basename in OT_allComms) then {
							...
							if(!(_basename in _abandoned) && _baseregion isEqualTo _townregion && _dist < 7500 && _stability < 50 && (random 100) > _chance) exitWith {
								_spend = _spend - 150;
								_done = true;
			...
			private _Spent = 0;
			{
				_x params ["_pos","_name","_pri","_gar"];
				private _numA = 0;
				private _numB = 0;
				private _dif = 0;
				private _priceA = 0;
				...
				if((_Spent < 500) && {(random 100 > 50)}) then {
					if !(_name in _abandoned) then {
						...
						_staticA = +OT_NATO_StaticGarrison_LevelOne;
						if(_pri > 500) then {
							_staticA = +OT_NATO_StaticGarrison_LevelTwo;
						};
						if(_pri > 1000) then {
							_staticA = +OT_NATO_StaticGarrison_LevelThree;
						};
						if(_name isEqualTo OT_NATO_HQ) then {
							_staticA = +OT_NATO_StaticGarrison_HQ;
						};
						if (_IsAirport) then {
							{
								_staticA pushBack _x
							}foreach(OT_NATO_Vehicles_StaticAAGarrison);
						};
						_numA = count _gar;
						_numB = count _staticA;
						_dif = _numB - _numA;
						_priceA = _dif * 50;
						if((_resources > _priceA) && {(_dif > 2)}) then {  // More than two garrisonvehicles missing, hasnt spent too much replacing garrisons
							_Spent = _Spent + _priceA;
							_resources = _resources - _priceA;
							_spend = _spend - _priceA;
							if !(_IsAirport) then {
								OT_objectiveData set [_found,[_x select 0,_x select 1,_x select 2,_staticA]];
							}else{
								OT_airportData set [_found,[_x select 0,_x select 1,_x select 2,_staticA]];
							};
							//systemChat format["%1 : %2 : %3 : %4",_name,_dif,_pri,_found]; - Debug Message
						};
					};
				};
			}foreach(OT_objectiveData + OT_airportData);
			...
			//Send the APC force if objective was attacked mid-game
			{
				_x params ["_pos","_name","_pri"];
				if !(_name in _abandoned) then {
					_HasDied = server getVariable format["GarrKilled%1",_name];
					if (_HasDied > 2) then {
						if ((_popControl > 1000) && {!(OT_NATO_SentAPC)} && {(_resources > 499)} && {(random 100 > 50)} && {(_pri > 499)}) then {
						_resources = _resources - 300;
						_frombase = _name;
						[_frombase] spawn OT_fnc_NATOAPCBrigade;
						OT_NATO_SentAPC = true;
						};
					};
			...
			//Upgrade FOBs
			{
				...
				if((_garrison < _max) && {(_spend > 150)} &&  {(random 100 > _chance)}) exitWith {
					...
					while {_count < 4} do {
						...
						_civ = _group createUnit [OT_NATO_Reinforcements call BIS_fnc_selectRandom, _start, [],0, "NONE"];
