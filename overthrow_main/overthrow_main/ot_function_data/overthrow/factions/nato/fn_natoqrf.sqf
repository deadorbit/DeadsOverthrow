...
//Send ground forces by land
	{
		...
		if(_pri > 500 && _popControl > (random [750,1000,2000])) then {
...
if(_strength >= 250) then {
	{
		...
		if(_pri > 600 && _strength >= 250) then {
			_ao = [_pos,_dir] call OT_fnc_getAO;
...
if(_strength > 300 && (count _air) > 0) then { //Send CAS
	if((_popControl > 500) or _strength > 1100)  then { //dont send a gunship if its the players first attack
...
//Send in CTRG if things look dim
if(count _ground > 1) then {
	if((_popControl > 1500 && _strength > 500) || (_popControl > 3000 && (random 100) > 25)) then {
		_obpos = (_ground select 1) select 0;
		_name = (_ground select 1) select 1;
		_dir = [_pos,_obpos] call BIS_fnc_dirTo;
		_ao = [_pos,_dir] call OT_fnc_getAO;
		[_obpos,_ao,_pos,450] spawn OT_fnc_CTRGalt; //Have it arrive as battle is ongoing to sneak in
	};
};
...
{OT_fnc_QRFStartHandler} remoteExec ["OT_fnc_QRFStartHandler", 0];
