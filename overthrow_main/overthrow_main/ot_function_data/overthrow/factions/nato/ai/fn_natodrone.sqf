    ...
    };
    {
        if((damage _x) < 1) then {
            [_x,_drone,_targets] call {
				...
				//Object is vehicle and not a civilian one
                if((count crew _x) > 0 && ((_x isKindOf "Car") || (_x isKindOf "Air") || (_x isKindOf "Ship")) && !(OT_allCivVehs getOrDefault [_ty, false])) exitWith {
