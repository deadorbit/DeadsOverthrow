...
if(!isNil "_close") then {
	...
	private _start = [_close, 0, 200, 1, 0, 0, 0] call BIS_fnc_findSafePos;
	private _priority = (_town in OT_NATO_priority);
	private _usecar = ((((_close distance _townPos) > 1500) && (random 100) > 50) || _priority);

	_group = creategroup blufor;
	_groups pushback _group;

	if (_usecar) then {
		private _vehCls = selectRandom OT_NATO_Vehicle_Police;
		_spawnpos = _start findEmptyPosition [5, 100, _vehCls];
		_veh = _vehCls createVehicle _spawnpos;
	...
	_civ = _group createUnit [OT_policeUnitCaptain, _start, [],0, "NONE"];
	...
	_civ = _group createUnit [OT_policeUnitRegular, _start, [],0, "NONE"];
