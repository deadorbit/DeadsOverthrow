...
[_unit] call {
	...
	if((random 100) > 80) exitWith {
		//This is a medic
		_unit addBackpack (OT_crim_backpacks call BIS_fnc_selectRandom);
	...
	private _drugchance = random 100;
	if(_drugchance > 97) exitWith {
		//This guy is a drug runner
		_unit addBackpack (OT_crim_backpacks call BIS_fnc_selectRandom);
		for "_i" from 1 to round(random 15) do {_unit addItemToBackpack "OT_Ganja";};
	};
	if(_drugchance < 20) exitWith {
		//This guy is a worse drug runner
		_unit addBackpack (OT_crim_backpacks call BIS_fnc_selectRandom);
		_unit addItemToBackpack "OT_Blow";
		_unit addItemToBackpack "OT_Ganja";
	};
};
