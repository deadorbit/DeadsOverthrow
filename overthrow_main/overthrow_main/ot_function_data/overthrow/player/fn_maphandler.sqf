...
private _gridSize = [_mapCtrl] call BIS_fnc_mapGridSize;
...
private _append = [];
		...
		if!(_side isEqualTo 1) then {
			...
			if!(isNil "_factionPos") then {
				if((_factionPos distance2D _mousepos) < 3000) then {
					...
					OT_VISIBLE_FACTIONS pushBackUnique [0.6, 0.5 ,_factionPos, _cls];
				};
...

	{
		...
		if((_tpos distance2D _mousepos) < 2500) then {
			...
			{
				_mapCtrl drawIcon [
					format["overthrow_main\ot_ui_data\ui\markers\shop-%1.paa",_x select 1],
					[1,1,1,1],
					_x select 0,
					0.2/_scale,
					0.2/_scale,
