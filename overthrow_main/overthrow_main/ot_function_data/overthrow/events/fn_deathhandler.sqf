...
if(isPlayer _me) exitWith {
	...
	if !(isMultiplayer) then {
		...
		if (_unit isEqualTo player) then {
			...
			/*_criminal = _killer getvariable "criminal";
...
call {
	if(!isNil "_civ") exitWith {
		...
		_standingChange = (round(_rep * 0.05) * -1);
		if(_standingChange < 50) then {//Remove either 5% of support or 50 support, which ever is higher
			_standingChange = -50;
		};
		[_town,-1] call OT_fnc_stability;
	};
	...
	if(!isNil "_criminal") exitWith {
		...
		_launcher = _me getVariable["launcher",false];
		_reward = 50;
		_stability = 2;
		_standingChange = 5;
		if(_civid > -1) then {
			...
			if(_gangid > -1) then {
				...
				if(_launcher) then {
					_gang set[10,(_gang select 10) - 1];
				};
			};
		};
	...
	if(!isNil "_crimleader") exitWith {
		...
		[_killer,_gangid,-999] call OT_fnc_gangRep;
	};
