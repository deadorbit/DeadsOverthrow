		...
		if((vehicle player) != player && count (player nearObjects [OT_portBuilding,30]) > 0) then {
			...
			if (player getVariable ["OT_tute_trigger",false]) then {
				...
				[] spawn {
					...
					sleep 3;
					hint "Go and speak to the local dealer. Head towards the marked location, you have nothing to worry about as long as you are not carrying/wearing any illegal items.";
			...
			if(vehicle player != player) exitWith {
				...
				call {
					...
					if([getPos player] call OT_fnc_positionIsAtWarehouse) exitWith {
			...
			if((_cTarget isKindOf OT_item_Storage || _cTarget iskindof "OT_Land_Cargo10_grey_F") && (_cTarget call OT_fnc_getOwner isEqualTo getPlayerUID player) && (vehicle player == player)) exitWith {
				call OT_fnc_containerdialog;
			};
