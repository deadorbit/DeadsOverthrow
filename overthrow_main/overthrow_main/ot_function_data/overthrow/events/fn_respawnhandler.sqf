...
private _town = _housePos call OT_fnc_nearestTown;
//player setPos (_housePos findEmptyPosition [1,20,typeof _new]);

private _battleLoc = server getVariable ["NATOattacking",""];
if (_town isEqualTo _battleLoc) then {
	_housePos = [_housepos, 50, 135, 0, 0, 45, 0, [], [_housePos,_housePos]] call BIS_fnc_findSafePos;
	_respawnMessage = _respawnMessage + " During a battle, you will respawn in a random radius around your spawn if it is set in the AO"
};
player setPos (_housePos findEmptyPosition [2,15,typeof _new]);
...
[] spawn {
	...
	player enablesimulation true;
	if (_respawnMessage isNotEqualTo "") then {
		systemChat _respawnMessage;
	};
};
