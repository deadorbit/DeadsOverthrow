params [["_target", objNull], ["_category", ""]];
...
if(_category isNotEqualTo "") then {
	...
	_validItems = OT_ShopStockData getOrDefault [_category, [""]];
	
	/*
	if(_category isEqualTo "Hardware") then {
		_validItems = ["OT_Steel","OT_Wood","OT_Plastic","OT_Fertilizer","OT_Ore"];
	};
	if(_category isEqualTo "Clothing") then {
		_validItems = OT_allLegalClothing + OT_allGlasses + OT_allGoggles + OT_allFacewear;
	};
	
	{
		if((_x select 0) isEqualTo _category) exitWith {
			{
				_validItems pushback _x;
			}foreach(_x select 1);
		};
	}foreach(OT_items);
	*/
...
private _allCargo = {
	...
	_myitems = _myitems - (keys OT_noCopyMags);
	_myitems
};
...
if !(isNil "_theseitems") then {
	{
		private _cls = _x;
		if(_category isEqualTo "" || _cls in _validItems) then {
			if(OT_has_TFAR) then {
