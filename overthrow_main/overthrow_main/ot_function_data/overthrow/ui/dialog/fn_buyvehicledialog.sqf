...
private _items = OT_vehTypes_civ;
...
if((_obpos distance player) < 250) then {
	if(_obname in (server getVariable ["NATOabandoned",[]])) then {
		...
		_items append [["EasyMG"]];
		if((_obname in OT_allAirports) || OT_debugMode) then {
			_items append OT_helis;
		}else{
			_items append OT_boats;
		};
		if(_obname == "Chemical Plant") then {
			_items append ((keys OT_allExplosives) + (keys OT_allDetonators));
		};
	}
};

if(OT_debugMode) then {
	_items = (keys OT_allExplosives) + (keys OT_allDetonators) + OT_helis + OT_vehTypes_civ + OT_boats + OT_staticBackpacks + [["Set_HMG"]] + [["EasyMG"]];
};
...
_textctrl ctrlSetStructuredText parseText format["<t size='1.7' shadow='2' align='left' valign = 'middle'>%1 | %2</t>",_name,"Vehicle Dealer"];

{
	_x params ["_cls"];
	if((_cls select [0,3]) != "IED") then {
		private _price = 0;
		if !(_cls == "Set_HMG" || _cls == "EasyMG" || _cls == "Set_SPG") then {
			_price = [_town,_cls,_standing] call OT_fnc_getPrice;
		};
		if("Fuel Depot" in (server getVariable "OT_NATOabandoned")) then {
			_price = round(_price * 0.75);
		};
		([_cls,_price] call {
			...
			if (_cls == "EasyMG") exitWith {
				private _p = ((cost getVariable "C_Offroad_02_unarmed_F") select 0) + 500;
				_p = _p + ((cost getVariable "LMG_03_F") select 0);
				_p = _p + 2500; //Convenience cost
				_price = _p;
				_name = "Jeep w/ LMG";
				_pic = "I_C_Offroad_02_LMG_F" call OT_fnc_vehicleGetPic;
				[_pic,_name,_price]
			};
			if (OT_allExplosives getOrDefault [_cls, false]) exitWith {
