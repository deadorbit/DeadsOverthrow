...
private _temperature = ambientTemperature select 0;
_temperature = _temperature * (9/5) + 32;//Farenheight
_temperature = [_temperature,1] call BIS_fnc_cutDecimals;

private _nearesttown = "";
_nearesttown = getpos player call OT_fnc_nearestTown;
...
private _extra = "";
((findDisplay 8001) displayCtrl 1609) ctrlSetTooltip "Lease this building";
...
_ctrl ctrlSetStructuredText parseText format[
	"
		...
		<t align='left' size='0.65'>Weather: %8 (Forecast: %9) | %14°F</t><br/>
		<t align='left' size='0.65'>Near: %15</t><br/>
		<t align='left' size='0.65'>Fuel Price: $%10/L</t><br/>
		<t align='left' size='0.65'>Opfor/Independent Legitamancy: %12|%13</t><br/>
		%11
	",
	...
	_extra,
	[server getVariable [format["legit%1","Opfor"],0], 1, 1] call CBA_fnc_formatNumber,
	[server getVariable [format["legit%1","Independant"],0], 1, 1] call CBA_fnc_formatNumber,
	_temperature,
	_nearesttown
];
...
if(typename _b isEqualTo "ARRAY") then {
	...
	_name = (_cls call OT_fnc_vehicleGetName) call OT_fnc_escapeXML;
	...
	if(_building call OT_fnc_hasOwner) then {
		...
		if(_owner isEqualTo getplayerUID player) then {
			...
			_buildingTxt = format["
				...
				<t align='left' size='0.65'>Lease Value: $%3/hr</t><br/>
				<t align='left' size='0.65'>Damage: %4%5</t><br/>",
				_name,
				_ownername,
		...
		if(typeof _building isEqualTo OT_flag_IND) then {
			...
			ctrlSetText [1609,"Remove"];
			((findDisplay 8001) displayCtrl 1609) ctrlSetTooltip "Removes this base and any FOB buildings built within its raidius without refunding.";
		...
		if((typeof _building) in OT_allRepairableRuins) then {
			...
			if(isNil "_price") then {
				...
				_buildingTxt = format["
					<t align='left' size='0.8'>%1</t><br/>
					<t align='left' size='0.65'>Lease Value: $%2/hr</t><br/>
					<t align='left' size='0.65'>Rating: %3</t>",
					_name,
					[_lease, 1, 0, true] call CBA_fnc_formatNumber,
					_size
				];
	...
	if((_ob select 1) isEqualTo "Business") then {
		...
		if(_obpos distance player < 250) then {
			if(_obname in (server getVariable ["GEURowned",[]])) then {
				...
				ctrlSetText [1201,OT_IMAGE_CLOSED_SIGN_WHITE];
				_areaText = format["
					<t align='left' size='0.8'>%1</t><br/>
