...
if(isNil "_stock") then {
	...
	}foreach(keys OT_allStaticBackpacks);
	*/

	private _tostock = [];
	while {_count < _numguns} do {
		private _type = selectRandom (keys OT_allWeapons);
	...
	while {_count < _numOptics} do {
		private _type = selectRandom (keys OT_allOptics);
		if !(_type in _tostock) then {
				_count = _count + 1;
...

{
	...
	if !(isNil "_cls" || (OT_noCopyMags getOrDefault [_cls, false])) then {
		private _txt = _cls;
		private _pic = "";
		...
		[_cls] call {
			...
			if(OT_allStaticBackpacks getOrDefault [_cls, false]) exitWith {
		...
		if (OT_allDrugs getOrDefault [_cls, false]) then {
			...
			if (OT_allOptics getOrDefault [_cls, false]) then {//Is Optic
