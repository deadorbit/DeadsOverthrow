...
_items = OT_Resources + OT_allItems + ["V_RebreatherIA"];
if(_town in (server getVariable ["NATOabandoned",[]]) || OT_debugMode) then {
	_items = OT_Resources + OT_allItems + OT_surplusStock + ["V_RebreatherIA"] + ["ACE_NVG_Gen1"] + (keys OT_allWeapons) + (keys OT_allMagazines) + OT_allAttachments + (keys OT_allStaticBackpacks) + (keys OT_allOptics) + OT_allVests + OT_allHelmets + OT_allClothing;
}else{
...
_numitems = 0;
{
	...
	if(tolower(_cls) find _SearchTerm > -1) then {
		...
		if !((_cls in _done) || (OT_allExplosives getOrDefault [_cls, false]) || (_cls == "OT_NatoParts") || (_cls == "OT_AGM")) then {
