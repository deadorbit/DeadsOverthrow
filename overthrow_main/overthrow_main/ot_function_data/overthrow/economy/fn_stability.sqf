...
private _stabilitychange = _this select 1;//Capitals should be significantly harder to negativly effect
if(_town in OT_capitals &&  !(_town in (server getVariable ["NATOabandoned",[]])) ) then {
	if(!(_stabilitychange == -1) && _stabilitychange < 0) then {
		_stabilitychange = round(_stabilitychange / 2);
	}else{
...
_stability = (server getVariable [format["stability%1",_town],0])+_stabilitychange;
