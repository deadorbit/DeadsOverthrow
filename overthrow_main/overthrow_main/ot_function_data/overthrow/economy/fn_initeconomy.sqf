...
if (server getVariable ["OT_economyScriptRan", false]) exitWith {}; //Only allow this script to be ran once

server setVariable ["OT_economyScriptRan", true, true];
...
OT_allPiers = [];
{
	...
	if(_pop > 1500) then {_pop = 1400 + round(random 500)};
    if(_pop > 800 && _pop < 1500) then {_pop = 700 + round(random 500)};
    if(_pop < 20) then {_pop = 30 + round(random 20)};
    private _stability = round(_base + random(20));

    //Low stability in small towns

    if((_pop < 200) && !(_name in OT_NATO_priority) && !(_name in OT_Capitals) && (_name in OT_spawnTowns)) then {
...
private _popVar=format["population%1",OT_mainCapital];
server setVariable [format["stability%1",OT_mainCapital],100,true];// Capital should start with max pop and stab
server setVariable [_popVar,2000,true];
...
if(_diff == 2) then {
	server setVariable [format["stability%1",_spawn],90,true];
}else{
	server setVariable [format["stability%1",_spawn],55,true];
};
