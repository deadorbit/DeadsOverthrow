...
private _dist = 50;
private _minShopDistance = 25; //Under no circumstances can two shops be this close
private _minSameShopDistance = 250; //Two of the same type of shop cant be closer than this

switch (true) do {
	case: (_town in OT_sprawling ||_town in OT_capitals) {
		_dist = 1000;
	};
	case: ()
	default {
...
private _storeTypes = keys _allShopStockData;
//Make sure the shop is physically closest
foreach ()


/* This is not a good way to do this
	if((count _shops) > (count _storeTypes) - 1) then {
		...
		{
			private _pos = getpos _x;
			//Get a random spot around the center, ensure shops arnt always in the exact same place each game
...
if((count OT_piers) > 0) then {
	{
		...
		if !(_po in _piers) then {
			...
			if(_do) then {
				_piers pushback _po;
				OT_allActivePiers pushback _po;
			};
		};
	}foreach(nearestObjects [_posTown,OT_piers, _dist,false]);
};
server setVariable [format["activepiersin%1",_town],_piers,true];
server setVariable [format["standing%1",OT_spawnFaction],20];//Give the player a free boost to starting faction reputation
