...
//Overwrites settings of certain object classes
([_typecls] call {
	params ["_typecls"];
	...
	if(_typecls == "Ammobox") exitWith {
		[
			...
			500,
			"An empty ammobox to fill with things you have acquired through... various means."
		]
	};
	if(_typecls == "Whiteboard") exitWith {
		[
			...
			[OT_item_Map],
			100,
			"Plan out your next assault from the middle of nowhere."
		]
	};
	//Go through the list of placeables, if it matches the object, use its settings
	{
...
if(_cost > 0) then {
	...
	if(
		...
		if ([getpos player,_typecls] call OT_fnc_canPlace) then {
			...
			if(_cls == "Land_CanisterFuel_Red_F") then {
				[modeTarget, 5] call ace_refuel_fnc_makeJerryCan;
				[modeTarget, true, [0, 1, 1], 10] call ace_dragging_fnc_setCarryable;
				[modeTarget, 1] call ace_cargo_fnc_setSize
			};
		}else{
			detach modeTarget;
			deleteVehicle modeTarget;
