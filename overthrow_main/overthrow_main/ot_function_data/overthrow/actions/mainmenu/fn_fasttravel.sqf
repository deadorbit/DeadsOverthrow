...
if(!OT_debugMode && OT_setting_fastTravelType > 1) exitWith {hint "Fast Travel is disabled"};
...
{
	if (OT_allDrugs getOrDefault [_x, false]) exitWith {_hasdrugs = true};
...
if((vehicle player) != player) then {
	...
	if(OT_setting_fastTravelType == 1) exitWith{hint "Current settings do not allow fast traveling while in a vehicle.";_exit=true};
	...
	if(OT_setting_fastTravelType >= 0 && ((typeOf(vehicle player)) in (OT_allVehicleThreats + OT_allHeliThreats + OT_allPlaneThreats)))  exitWith {hint "You cannot fast travel in an offensive vehicle";_exit=true};  //Merge from Rudyblah vrsn
};
if(_exit) exitWith {};
...
OT_FastTravel_MapSingleClickEHId = addMissionEventHandler ["MapSingleClick", {
	...
	if !(_validob) then {
		if (!OT_debugMode && !(_pos inArea _region)) then {
