...
if((OT_allExplosives getOrDefault [_cls, false]) && (_chems < (_cost select 3))) exitWith {format["You need %1 chemicals",_cost select 3] call OT_fnc_notifyMinor};
...
if(_cls isKindOf ["Default",configFile >> "CfgMagazines"]) exitWith {
	...
	if (OT_allExplosives getOrDefault [_cls, false]) then {
...
private _target = player getVariable ["OT_shopTarget","Self"];

//Check for execptions to adding items
switch (true) do {
	case(_target isEqualTo "Vehicle") : {
		if (!(_veh canAdd [_cls,_amnt])) then {
			"This vehicle is full." call OT_fnc_notifyMinor;
		};
	};
	case(_cls isKindOf "Bag_Base") : {
		if(backpack player isNotEqualTo "") then {
			"You already have a backpack" call OT_fnc_notifyMinor;
		};
	};
	case(!(player canAdd [_cls,_amnt])) : {
		"There is not enough room in your inventory" call OT_fnc_notifyMinor;
	};
	default {
		_handled = true;
...
if(_handled) then {
	...
	[-_price] call OT_fnc_money;
	//Investagate if evil transaction seen by popo
	if ([_cls] call OT_fnc_isIllegal) then {
	...
	switch (true) do {
		case(_target == "Vehicle") : {
			[vehicle player, _cls, _amnt] call DEDF_fnc_simpleAddAny;
		};
		case([_cls] call DEDF_fnc_isBackpack) : {
