//Price parameters
private _basecost = 15; //Minimum price of vest
private _armorMult = 2.5; //Multiplier per armor value to cost of vest
private _spaceMult = 0.5; //Multiplier per space value to cost of vest
private _legalMult = 0.5; //multiplier to cost of LEGAL vests
private _weightMult = 0.1; //Multiplier to how much mass REDUCES the price of a vest
private _numHitPointsMult = 0.5; //Multiplyer to the value of each hitpoint (which gets added onto armorMult)
/*
	Price equation = ((armorValue * (armorMulti + (numHitPoints * numHitPointsMulti))) + (spaceValue * spaceMulti) max baseCost) * legalMult
*/
//Resource Parameters, Note: legal vests are assumed made of cloth, illegal made of steel
private _metalThreshold = 50; //How much armor for cloth to be converted to metal?
private _clothToMetal = 0.75; //If a civilian passes the threshold, how much steel becomes cloth?
...
//Sadly there is no simple and reliable way to calculate this, so hand dictating must be done (maybe could make a graphical system)
private _legalVestsExeptions = [
...
//Tells the thing these are supposed to be classed as legal/illegal (wont warn about them)
private _vestsNoWarning = [
	"eo_eodvest_1",
	"eo_eodvest_2",
	"eo_eodvest_3",
	"eo_tacvest_1",
...
private _allvests = format
	["
		(configname _x) isKindOf [""Vest_Camo_Base"", configFile >> ""CfgWeapons""]
		||
		(configname _x) isKindOf [""Vest_NoCamo_Base"", configFile >> ""CfgWeapons""]
	"] 
...
private _buildCfg = true; //If false, gives debug data, else gives configs
...
if !(_buildCfg) then {
	_averageCost = _averageCost / _totalConfigs;

	_debug = _debug + "__DEBUG_INFO__" + _newline + format[
		"Total Configs: %1
		Average Price: %2
		Max Price: %3
		Min Price: %4",
	...
	_output = _output + _newline + _debug + _warning;
};
