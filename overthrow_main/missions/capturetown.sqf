...
private _jobsearch = {_x == _jobid} count _activeJobs;

if(_jobsearch >= 2) exitwith{//This is a duplicate of this job, cancel it
	private _index = _activejobs find _jobid;
	_activeJobs deleteat _index;
	spawner setvariable ["OT_activeJobIds",_activeJobs,true];
	[]
};
            ...
            [
                {
                    ...
                    private _factionName = server getvariable format["factionname%1",_faction];
                    format ["Incoming message from %1: %2 captured. Donating $10,000 to the resistance and will help with efforts to ease the local populace. (+10 stability, +10 %1)",_factionName,_destinationName] remoteExec ["OT_fnc_notifyMinor",0,false];
                },
