class CfgOverthrowMissions
{
    ...
    class Tagging
    {
        target = "Town";
        repeatable = 0;
        condition = "params['','','_town'];!(_town in (server getVariable ['NATOabandoned',[]])) && ((server getVariable [format['tagsin%1',_town],0]) <= 14)";
    ...
    class ReconBase
    {
        target = "Base";
        repeatable = 0;
        condition = "params['_inSpawnDistance', '_name'];!(_name in (server getVariable ['NATOabandoned',[]])) && (server getVariable [format[""garrison%1"",_name],0]) > 0 && !(_name in (server getVariable ['ReconnedBases',[]]))";
...
//New jobs
	...
	class DrugCar
    {
        ...
        repeatable = 1;
        condition = "params['','','_town'];(_town in (server getVariable ['NATOabandoned',[]]))";
        script = "\overthrow_main\missions\DrugCar.sqf";
        chance = 5;
        expires = 12;
        requestable = 1;
    };
	...
	class Revolution
    {
        ...
        repeatable = 1;
        condition = "params['','','_town'];(_town in (server getVariable ['NATOabandoned',[]]))";
        script = "\overthrow_main\missions\Revolution.sqf";
        chance = 5;
        expires = 12;
        requestable = 1;
    };
	...
	class Chaos
    {
        ...
        repeatable = 0;
        condition = "params['','','_town'];!(_town in (server getVariable ['NATOabandoned',[]]))";
        script = "\overthrow_main\missions\chaos.sqf";
        chance = 25;
        expires = 6;
        requestable = 1;
    };
	
	class ShopBoxTransport
    {
        target = "Shop";
        repeatable = 1;
        condition = "true";
        script = "\overthrow_main\missions\shopboxtransport.sqf";
        chance = 100;
        expires = 24;
        requestable = 1;
    };
