...
private _count = 0;
private _reward = 1000;
_params = [_base,_count];
private _basevar = OT_objectiveData findif {(_x select 1) isEqualTo _base};
_basevar = (OT_objectiveData select _basevar) select 2;
if(_basevar != -1) then {
	_reward = _basevar + round(random(_basevar)) + 1000;
};
private _effect = format["<t size='0.9'>Reward: $%1 to player closest to base</t>",_reward];
...
[
    ...
    {
        //Success Check
        params ["_base","_oldcount","_reward"];
                ...
                if((vehicle leader _group) == leader _group) then {
                    {
                        ...
                        _seen = ((resistance knowsAbout (leader _group)) > 0.5);
                    ...
                    if ((vehicle leader _group) isKindOf "StaticWeapon") exitWith {}; // too hard to spot
                    {
                        ...
                        _seen = ((resistance knowsAbout (vehicle leader _group)) > 1);
    ...
    {
        params ["_base","_count","_reward","_wassuccess"];
 
        //If mission was a success
        if(_wassuccess) then {
			private _recconed = server getVariable ["ReconnedBases",[]];
			_recconed pushbackunique _base;
			server setVariable ["ReconnedBases",_recconed];
            private _loc = server getVariable _base;
            private _players = [];
            {
                if(isPlayer _x && alive _x) then {
